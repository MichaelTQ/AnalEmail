...
- 最后更新：2026-07-18
- 当前阶段：第三阶段——接入 Ollama 和 Qwen，完成单封邮件结构化分析
- 代码状态：阶段 1、2 已完成；阶段 3 最小闭环已实现
- 离线测试：已通过
- 真实邮箱测试：已定位并修复 163 `Unsafe Login`，只读获取最近 10 封邮件已通过
- 当前明确不包含：批量 AI 分析、数据库、网页界面、Docker、AnythingLLM、自动回复或发送
...
13. 使用本地 Ollama 的 `qwen3:latest` 分析最新一封成功解析的邮件。
14. 固定输出摘要、分类、重要程度、待办和截止日期，并校验 JSON 结构。
15. Ollama 连接失败、模型不存在或输出格式错误时显示中文错误。
...
main.py
  ...
  ├── 对每封邮件调用 email_parser.parse_email()
        ...
        ├── 提取纯文本或 HTML 正文
  │     └── 检查附件
  └── 将第一封成功解析的邮件交给 ai_analyzer.analyze_email()
        ├── 构造严格 JSON Prompt
...
- 同时读取 `OLLAMA_BASE_URL` 和 `OLLAMA_MODEL`，模型名不写死在业务代码中。
...
- `_build_prompt()`：构造只允许五个固定字段的邮件分析 Prompt。
- `analyze_email()`：使用标准库请求本地 Ollama `/api/chat`，目前只分析最新一封邮件。
- `_validate_analysis()`：校验摘要、分类、重要程度、待办和截止日期的类型及取值。
- 最多向模型传入正文前 12000 个字符；该限制只影响 AI 输入，不影响邮件解析结果。
...
- 顶层配置或连接错误返回退出码 `1`。
- 单封邮件失败不会中断循环。
- 全部解析成功返回 `0`；存在单封解析失败时返回 `2`；AI 分析失败返回 `3`。
...
- 本地 `qwen3:latest` 已用虚构会议邮件完成真实调用，返回的五字段 JSON 通过校验。
...
现有 `.env` 已包含邮箱和 Ollama 配置，不要再次复制模板覆盖它。直接运行：
...
1. 终端能显示 `.env` 中 `EMAIL_LIMIT` 指定数量以内的最近邮件。
2. 中英文标题、发件人和正文可读。
3. HTML 邮件中没有明显 HTML 标签。
4. 附件没有混入正文。
5. 运行后未读邮件仍保持未读。
6. 邮件解析统计数量符合实际显示数量。
7. 最后出现“最新邮件 AI 分析”，且包含五个固定字段。
8. `.env` 未被 Git 跟踪。
